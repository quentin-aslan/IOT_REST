...
router.post("/dht22", async (req, res, next) => {
    try {
        ...
        const humidity = parseFloat(datas.humidity).toFixed(2);
        const temperature = parseFloat(datas.temperature).toFixed(2);
        const realFeel = parseFloat(datas.realFeel).toFixed(2);
